...
class PatchDropout(nn.Module):
    ...
    def forward(self, x, B, T):
        ...
        if T == 1:
            ...
            patch_indices_keep = rand.topk(num_patches_keep, dim=-1).indices
            patch_indices_keep = patch_indices_keep.unsqueeze(
                1).repeat(1, T, 1)
            patch_indices_keep = rearrange(
                patch_indices_keep, 'b t n -> (b t) n')
...
class CLIPEncoderLayer(nn.Module):
    def __init__(self, config: LanguageBindVideoConfig):
        ...
        if self.add_time_attn:
            ...
            nn.init.normal_(self.temporal_embedding,
        ...
        if self.add_time_attn:
            ...
            if t != 1:
                ...
                hidden_states = rearrange(
                    hidden_states, '(b t) n d -> (b n) t d', t=t)
                hidden_states = hidden_states + \
                    self.temporal_embedding[:, :t, :]
                hidden_states = rearrange(
            ...
            hidden_states = residual + \
                rearrange(hidden_states, '(b n) t d -> (b t) n d', n=n)
...
class CLIPPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, CLIPTextEmbeddings):
            module.token_embedding.weight.data.normal_(
                mean=0.0, std=factor * 0.02)
            module.position_embedding.weight.data.normal_(
                mean=0.0, std=factor * 0.02)
        elif isinstance(module, CLIPVisionEmbeddings):
            factor = self.config.initializer_factor
            nn.init.normal_(module.class_embedding, mean=0.0,
                            std=module.embed_dim**-0.5 * factor)
            nn.init.normal_(module.patch_embedding.weight,
...
class CLIPVisionTransformer(nn.Module):
    ...
    def forward(
        ...
        #################################
        encoder_outputs.hidden_states = [rearrange(
            i, '(b t) n c -> b t n c', b=B) for i in encoder_outputs.hidden_states]
        if not return_dict:
            return (last_hidden_state, pooled_output) + encoder_outputs[1:]
...
class LanguageBindVideo(CLIPPreTrainedModel):
    ...
    def convert_to_lora(self):
        ...
        self.vision_model.encoder = get_peft_model(
    ...
    def resize_pos(self, m, vision_config):
        ...
        if vision_config.num_mel_bins != 0 and vision_config.target_length != 0:
            m.image_size = [vision_config.num_mel_bins,
                            vision_config.target_length]
        m.config.image_size = [m.image_size, m.image_size] if isinstance(
            m.image_size, int) else m.image_size
        # pos resize
        ...
        grid_size = [m.config.image_size[0] // m.patch_size,
        ...
        m.register_buffer("position_ids", torch.arange(
            m.num_positions).expand((1, -1)))
    ...
    def forward(
        ...
        logits_per_text = torch.matmul(
            text_embeds, image_embeds.t()) * logit_scale
